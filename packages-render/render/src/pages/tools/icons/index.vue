<template>
    <div class="p-25px h-1/1 overflow-auto scrollbar">
        ...
        <div class="mt-10px">图片保存重命名，<code>$w$</code>替换为宽度，<code>$h$</code>替换为高度</div>
        <div class="flex items-center control mt-[8px]">
            <input class="input !w-280px" type="text" v-model="saveFileFormat" placeholder="保存格式">
        </div>
        ...
        <div class="mt-6px tags buttons icon-generate ios">
            <template v-for="(item) in iphoneIcons">
        ...
        <div class="mt-6px tags buttons icon-generate android">
            <template v-for="(item) in androidIcons">
                <Tag :src='state.curFile' :render-type="state.renderType" :width="item[0]" :height="item[1]"></Tag>
            </template>
        </div>
...
const saveFileFormat = ref("icon-$w$x$h$")
provide(Token, saveFileFormat)
