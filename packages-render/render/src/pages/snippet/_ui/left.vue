                ...
                if(res==1){
                    if (state.activeKeys?.includes(data.key)) {
                        try {
                            ...
                            await _agent.call("sapi.snippet.dels", r)
...
const dropFn = (status: ENiuTreeStatus, data: INiuTreeData<any>, targetData: INiuTreeData<any>): Promise<boolean> => {
    return new Promise(async resolve => {
        try {
            isLoading.value = true
            await _agent.call("sapi.snippet.move", status, data.key, targetData.key)
            isLoading.value = false
            resolve(true)
        } catch (error) {
            console.error(error)
...
async function handleDelete(data: INiuTreeData) {
    try {
        ...
        await _agent.call("sapi.snippet.dels", keys)
        isLoading.value = false
        data.isDel = true
        removeByKey(data.key, list.value)
...
async function handleRename(data: INiuTreeData, done: (status?: boolean) => void) {
    try {
        isLoading.value = true
        await _agent.call("sapi.snippet.rename", data.key, data.title)
...
function onExpand(node: INiuTreeData) {
    _agent.call("sapi.snippet.expand", node.key, node.isExpand)
}
/**
 * 处理创建回调
 */
async function handleCreateOne(data: INiuTreeData, parent: INiuTreeData | undefined, done: (status: boolean) => void) {
    try {
        isLoading.value = true
        await _agent.call("sapi.snippet.add", toRaw(data), parent?.key)
        state.isFocus = true
...
onBeforeMount(() => {
    ;(async () => {
        let _list = await _agent.call("sapi.snippet.readData")
